...
def apply(event_log: EventLog, parameters: Optional[Dict[Any, Any]]):
    ...
    dbscan = DBSCAN(eps=parameters[EPS], min_samples=parameters[MIN_SAMPLES], metric=parameters[METRIC]).fit(average_similarity_matrix)
    ...
    activity_to_class_lp = map_activities_to_classes(trace_list, dbscan.labels_, average_similarity_matrix)
