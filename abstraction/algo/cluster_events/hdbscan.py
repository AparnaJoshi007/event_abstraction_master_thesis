...
from abstraction.algo.constants import DEFAULT_ACTIVITY_KEY, DEFAULT_LIFECYCLE_KEY, EPS, MIN_SAMPLES, METRIC, START_LIFECYCLE_VALUE
...
def apply(event_log: EventLog, parameters: Optional[Dict[Any, Any]]):
    if parameters is None:
        parameters = {EPS: 0.5, MIN_SAMPLES: 5, METRIC: 'precomputed'}

    po_graphs_per_case, average_similarity_matrix = EventEncoding.apply(event_log)

    hdbscan = hdb.HDBSCAN(cluster_selection_epsilon=parameters[EPS], min_cluster_size=parameters[MIN_SAMPLES], metric=parameters[METRIC]).fit(cluster_encoding)
    ...
    activity_to_class_lp = map_activities_to_classes(trace_list, hdbscan.labels_, average_similarity_matrix)
